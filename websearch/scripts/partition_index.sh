...
function find_parts() {
  ...
  echo $(ls ${src_base_dir} | sort | while read line; do echo ${src_base_dir}/${line}; done)
}
...
function usage() {
  local progname=$1
  echo -e "usage: ${progname} <parts_count> <partitions_count> <partition_id> <src_base_dir> <dest_base_dir>"
  echo -e ""
  ...
  echo -e "  <dest_base_dir>\tdestination directory that corresponds to the partition"
  exit 1
}
